...
def clustering(Cluster, r, subsequence):
    ...
    if cluster_id != False:
        if len(Cluster.clusters[cluster_id]) < Cluster.nb_clustroid and not any(np.array_equal(subsequence, i) for i in Cluster.clusters[cluster_id]):
            ...
            dist_matrice = np.array([[z_norm_dist(
                i, j) for i in Cluster.clusters[cluster_id]] for j in Cluster.clusters[cluster_id]])
...
def score_to_label(nbr_anomalies, scores, gap):
    ...
    while len(real_indices) < nbr_anomalies and len(tmp) != 1:
        ...
        indices = [i for i, val in enumerate(
            tmp) if val == threshold]  # tmp.index(max(tmp))
    ...
    # [0 if i<threshold else 1 for i in scores ]
...
class class_our:
    ...
    def test(dataset, X, right, nbr_anomalies, gap, scoring_metric="merlin"):
        ...
        def scoring(scores):
            ...
            if scoring_metric == "nab":
                ...
                for element in right:
                    ...
                    real_label_frame = pd.DataFrame(
                        real_label, columns=['changepoint'])
                    scores_frame = pd.DataFrame(
                        scores, columns=['changepoint'])
                    real_label_frame["datetime"] = pd.to_datetime(
                        real_label_frame.index, unit='s')
                    scores_frame["datetime"] = pd.to_datetime(
                        scores_frame.index, unit='s')
                    real_label_frame = real_label_frame.set_index('datetime')
                    scores_frame = scores_frame.set_index('datetime')
                nab_score = evaluating_change_point(
                    [real_label_frame.changepoint], [scores_frame.changepoint])
        ...
        space2 = {"window": hp.choice("window_index", possible_window), "threshold": hp.choice(
            "threshold_index", possible_threshold)}
        trials = Trials()
      ...
      real_scores= our(X,w=possible_window[best["window_index"]], r=possible_threshold[best["threshold_index"]] )
        ...
        best_param = best_param = {
            "window": possible_window[best["window_index"]], 'threshold': possible_threshold[best["threshold_index"]]}
